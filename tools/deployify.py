...
'''  // Shared backend storage: everyone with the passcode reads/writes the same
  ...
  async function attemptUnlock(code, fromLink) {
    ...
    if (result.ok) {
      // Safety: a full-access (editor) passcode must never travel in a link. If one is
      // used via ?k=, refuse to auto-open and don't persist it -- only read-only viewer
      // passcodes are allowed to ride in a shareable URL.
      if (fromLink && !result.readOnly) {
        try { history.replaceState(null, "", location.pathname); } catch (e) {}
        showGate("For safety, a full-access passcode can't open from a link. Enter it manually here, and use your read-only viewer passcode for shareable links.");
        return;
      }
      PASSCODE = code;
      if (!fromLink) localStorage.setItem("bft_passcode", code);
  ...
  function initGate() {
    ...
    // ?k=<passcode> in the link auto-unlocks (meant for the read-only viewer passcode, so a
    // shared link opens with one click). Takes priority over a remembered passcode.
    const linkKey = new URLSearchParams(location.search).get("k");
    const remembered = localStorage.getItem("bft_passcode");
    if (linkKey) attemptUnlock(linkKey, true);
    else if (remembered) attemptUnlock(remembered, false);
    else showGate();
